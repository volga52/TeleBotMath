...
class HandlerFSM(Handler):
    ...
    def __init__(self, dp):
        ...
        self.math_element = MaticBotElem()
        self.math_cod = None
    ...
    async def math_init(self):
        ...
        self.math_element.launch(self.math_cod)
        # Определение количества уравнений
        quantity_tests = self.math_element.quantity_equations
    ...
    async def first_state_equation(self):
        ...
        self.math_element.message_dict['equation'] = FIRST_EXC_ANSWER[0]
        self.math_element.message_dict['answer'] = FIRST_EXC_ANSWER[1]
        # print(self.math_element.message_dict)
    ...
    async def excerpt_state_equation(self, message: types.Message,
                                     state: FSMContext):
        """Вывод фразы-цитаты"""
        if self.math_element.check_ratio_print_ex():
            text = self.math_element.message_dict.get('excerpt', 'None')
    ...
    async def test_state_equation(self, message: types.Message,
        ...
        try:  # Если получили число
            ...
            if response == self.math_element.message_dict['answer']:
        ...
        self.math_element.count_ratio_correct_answer(reply_text[1])
    ...
    async def new_equation(self, message: types.Message, state: FSMContext):
        if message.text == YES:
            try:
                ...
                self.math_element.get_main()
                value_dict = self.math_element.message_dict
                # print(value_dict)
        ...
        equation = self.math_element.message_dict.get('equation', 'None')
